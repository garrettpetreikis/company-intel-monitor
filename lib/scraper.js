...
Be ruthlessly sales-focused. Every signal must connect to a cylinder sales opportunity.
...
async function analyzeCompetitor(company, rawData) {
  const prompt = `You are an expert competitive intelligence analyst helping a hydraulic cylinder salesperson identify weaknesses, vulnerabilities, and openings created by their competitors.
...
- opportunity: one specific action to exploit this opening (e.g. "Call any account currently buying from them and lead with your on-time delivery record")
- severity: "high","med","low" (how big is the opening this creates for you)
- source_date: publish date of the source in "Month YYYY" format (e.g. "March 2026") — extract from the date shown next to the article title in the raw data. If no date is available, use null
- sources: array of source names

Be direct and opportunistic. Every signal must translate into a specific sales action.
Return ONLY a valid JSON array, no markdown. If no relevant signals: []`;
...
async function scrapeCompany(company) {
  ...
  const signals = await analyzeAccount(company, { googleNews, prNewswire, businessWire, tradePress, jobs, thomasnet, website, edgar });
...
async function scrapeCompetitor(company) {
  ...
  const [googleNews, prNewswire, businessWire, tradePress, jobs, thomasnet, website, edgar, reviews] =
    await Promise.all([
  ...
  const signals = await analyzeCompetitor(company, { googleNews, prNewswire, businessWire, tradePress, jobs, thomasnet, website, edgar, reviews });
  console.log(`  ${signals.length} competitor signals found for ${company.name}`);
  return { ...company, signals, scrapedAt: new Date().toISOString() };
}
